...
### all charts will have unique URI, will have to change parameters
...
class Scraper
  attr_reader :results, :artists, :songs
# client = Soundcloud.new(:client_id => '02gUJC0hH2ct1EGOcYXQIzRFU91c72Ea')
  ...
  def self.scrape_from_api(endpoint_url)
    ...
    # elsif (endpoint_url.has_key?("kind".to_sym) && (endpoint_url["kind".to_sym] == "track"))
    #   @@songs << endpoint_url
    # end
    ...
    if h.has_key?("kind".to_sym) && h["kind".to_sym] == "user"
      ...
      @@results << h
    elsif h.has_key?("kind".to_sym) && h["kind".to_sym] == "track"
      @@songs << h
    end
...
trending_all = Scraper.scrape_from_api("https://api-v2.soundcloud.com/charts?kind=trending&genre=soundcloud:genres:all-music&client_id=02gUJC0hH2ct1EGOcYXQIzRFU91c72Ea&limit=20&offset=0&linked_partitioning=1")
binding.pry
puts trending_all
